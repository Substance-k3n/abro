...
- [x] `internal/money`: port of `packages/types/src/{money,split,debt-simplification}.ts`
      ...
      passing.
- [ ] SQL schema + `golang-migrate` migrations translated from
      `apps/api/prisma/schema/*.prisma`.
- [ ] `sqlc` config + generated typed queries wired to a real dev
      Postgres.
- [ ] `internal/httpx`: session-auth middleware, JSON error-response
      helper (mirrors `common/filters/http-exception.filter.ts`),
      request-scoped user context (mirrors `@CurrentUser()`).
- [ ] `cmd/api/main.go`: router wiring, graceful shutdown, config from
      env.
...
`OAuthAccount` tables port directly from `auth.prisma`.
